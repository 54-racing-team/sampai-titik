...
public final class JourneyPageMainVM {
    ...
    public func startTrackingIfPossible(modelContext: ModelContext) async {
        guard !isTrackingStarted,
            ...
            let destination = stationDTO(named: destinationName)
        else { return }
        ...
        trackingViewModel.locationManager.onLocationUpdate = {
    ...
    private func advanceToStation(index newIndex: Int) {
        ...
        stations[newIndex].type =
            (newIndex == stations.count - 1) ? .destination : .current
        ...
        // Dynamci island tracking
        LiveActivityManager.shared.updateJourneyActivity(
            currentStationCode: stationDTO(named: self.currentStationName)?.id ?? "-",
            currentStationName: self.currentStationName,
            nextStationCode: stationDTO(named: self.nextStationName)?.id ?? stationDTO(named: self.destinationName)?.id ?? "-",
            nextStationName: self.nextStationName,
            isSoundEnabled: isReminderActive
        )
